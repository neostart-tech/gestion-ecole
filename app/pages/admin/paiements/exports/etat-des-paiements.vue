<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 lg:p-8 transition-colors">
    <!-- En-tête -->
    <div class="mb-8">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Export des paiements</h1>
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Générez des rapports détaillés sur les paiements</p>
          </div>
        </div>

        <!-- Stats rapides -->
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-3 px-4 py-2 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-2">
              <span class="w-2 h-2 bg-green-500 rounded-full"></span>
              <span class="text-sm text-gray-600 dark:text-gray-300">{{ etudiantStore.nombreEtudiant }} étudiants</span>
            </div>
            <div class="w-px h-4 bg-gray-200 dark:bg-gray-700"></div>
            <span class="text-sm text-gray-600 dark:text-gray-300">{{ niveauStore.niveaux.length }} niveaux</span>
            <div class="w-px h-4 bg-gray-200 dark:bg-gray-700"></div>
            <span class="text-sm text-gray-600 dark:text-gray-300">{{ filiereStore.filieres.length }} filières</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-4">
                <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                ...
                <div v-if="exportStore.exportParams.type === 'niveau'">
                  <label class="block text-sm text-gray-600 dark:text-gray-400 mb-1">Niveau <span class="text-red-400">*</span></label>
                  <Dropdown
                ...
                <div v-else-if="exportStore.exportParams.type === 'filiere'">
                  <label class="block text-sm text-gray-600 dark:text-gray-400 mb-1">Filière <span class="text-red-400">*</span></label>
                  <Dropdown
                    v-model="exportStore.exportParams.id"
                ...
                <div v-else-if="exportStore.exportParams.type === 'etudiant'">
                  <label class="block text-sm text-gray-600 dark:text-gray-400 mb-1">Étudiant <span class="text-red-400">*</span></label>
                  <Dropdown
                    v-model="exportStore.exportParams.id"
                    ...
                    <template #value="slotProps">
                      <div v-if="slotProps.value" class="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div class="text-xs text-gray-500 dark:text-gray-400">{{ getEtudiantMatricule(slotProps.value) }}</div>
                        </div>
                      </div>
                    </template>
                  </Dropdown>
                ...
                <div v-else class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p class="text-sm font-medium text-blue-800 dark:text-blue-300">Export global</p>
                  <p class="text-xs text-blue-600 dark:text-blue-400">Tous les étudiants seront inclus</p>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-3">
                  <div>
                    <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Date début</label>
                    <Calendar v-model="periodeDebut" dateFormat="dd/mm/yy" placeholder="jj/mm/aaaa" class="w-full" :showIcon="true" />
                  </div>
                  <div>
                    <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Date fin</label>
                    <Calendar v-model="periodeFin" dateFormat="dd/mm/yy" placeholder="jj/mm/aaaa" class="w-full" :minDate="periodeDebut" :showIcon="true" />
                  </div>
                </div>

                <!-- Aperçu des filtres -->
                <div v-if="exportStore.isValidExport" class="mt-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                  ...
                  <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    <div class="flex justify-between">
                      <span>Type :</span>
                      <span class="font-medium text-gray-700 dark:text-gray-300">{{ exportStore.exportTypeLabel }}</span>
                    </div>
                    <div v-if="exportStore.selectedItemName" class="flex justify-between mt-1">
                      <span>Sélection :</span>
                      <span class="font-medium text-blue-600 dark:text-blue-400">{{ exportStore.selectedItemName }}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div class="lg:col-span-4 space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div class="space-y-2">
            <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/50 rounded">
              <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-gray-600 dark:text-gray-300">Montant initial par étudiant</span>
            </div>
            <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/50 rounded">
              <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-gray-600 dark:text-gray-300">Montant après bourse</span>
            </div>
            <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/50 rounded">
              <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-gray-600 dark:text-gray-300">Total payé / Reste à payer</span>
            </div>
            <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/50 rounded">
              <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-gray-600 dark:text-gray-300">Statut (soldé/en cours/retard)</span>
            </div>
            <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/50 rounded">
              <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-gray-600 dark:text-gray-300">Ligne de totaux généraux</span>
            </div>
          </div>
