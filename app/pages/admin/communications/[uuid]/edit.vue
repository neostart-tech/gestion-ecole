<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500 mt-1">Mettez à jour les détails de la communication.</p>
    </div>
        ...
        <div class="bg-white p-6 rounded-[5px] shadow-sm border border-gray-100">
          <div class="space-y-4">
            ...
            <div v-if="existingAttachments.length > 0">
              <label class="block text-sm font-medium text-gray-700 mb-2">Pièces jointes existantes</label>
              <div class="space-y-2">
                <div v-for="file in existingAttachments" :key="file.id" class="flex items-center justify-between p-2 bg-gray-50 rounded-[5px] border border-gray-100">
                  <div class="flex items-center">
                    ...
                    <a :href="file.file_path" target="_blank" class="text-sm text-violet-600 hover:underline truncate max-w-xs">{{ file.original_name }}</a>
                  </div>
                  <Can action="update-communication">
                    <button @click="deleteExistingAttachment(file.id)" class="text-red-500 hover:text-red-700" title="Supprimer">
            ...
            <div class="pt-4 border-t border-gray-100">
              <label class="block text-sm font-medium text-gray-700 mb-1">Ajouter des pièces jointes</label>
              ...
              <div v-if="files.length > 0" class="mt-4 space-y-2">
                <div v-for="(file, index) in files" :key="index" class="flex items-center justify-between p-2 bg-violet-50 rounded-[5px] border border-violet-100">
                  <div class="flex items-center">
                    <svg class="w-4 h-4 text-violet-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 012 0h4a1 1 0 110 2H8a1 1 0 01-1-1z" clip-rule="evenodd" />
                    </svg>
                    <span class="text-sm text-violet-900 truncate max-w-xs">{{ file.name }}</span>
                  </div>
                  <button @click="removeFile(index)" class="text-violet-500 hover:text-violet-700">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white p-6 rounded-[5px] shadow-sm border border-gray-100">
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              ...
              <span 
                class="px-2 py-1 text-xs font-bold rounded-[5px]"
            ...
            <div class="flex items-center justify-between pt-2">
              <span class="text-sm text-gray-700">Changer de statut</span>
              <button 
            ...
            <p v-if="!initialIsPublished && form.is_published" class="text-[10px] text-green-600 mt-1">
              Les notifications seront envoyées dès l'enregistrement.
            </p>
...
const initialIsPublished = ref(false);
...
onMounted(async () => {
  ...
  // Dans un cas réel, on ferait un fetch spécifique par UUID ou on chercherait dans la liste
  // Pour cet exemple, on cherche dans la liste chargée
  if (communicationStore.communications.length === 0) {
    await communicationStore.fetchAdminCommunications();
  }
  
  const comm = communicationStore.communications.find(c => c.uuid === uuid);
  
  if (comm) {
    form.value.title = comm.title;
    form.value.content = comm.content;
    form.value.type = comm.type;
    form.value.target_type = comm.target_type;
    form.value.target_data = Array.isArray(comm.target_data) ? comm.target_data : [];
    form.value.is_published = comm.is_published;
    initialIsPublished.value = comm.is_published;
    
    if (comm.expires_at) {
        // Format pour l'input datetime-local
        const date = new Date(comm.expires_at);
        form.value.expires_at = date.toISOString().slice(0, 16);
    }
    
    if (comm.target_type === 'specific_users' && Array.isArray(comm.target_data)) {
      const dataIds = comm.target_data.map((id: any) => Number(id));
      selectedUsers.value = userStore.users.filter(u => dataIds.includes(Number(u.id)));
    }
    
    existingAttachments.value = comm.attachments || [];
  } else {
    Swal.fire({
      icon: 'error',
      title: 'Erreur',
...
watch(() => form.value.target_type, (newVal, oldVal) => {
  if (oldVal !== undefined) {
...
const deleteExistingAttachment = async (id: number) => {
  try {
    ...
    if (result.isConfirmed) {
      await communicationStore.deleteAttachment(id);
...
const submit = async () => {
  ...
  try {
    ...
    await communicationStore.updateCommunication(uuid, form.value);
    
    // 2. Uploader les nouveaux fichiers s'il y en a
    if (files.value.length > 0) {
      await communicationStore.uploadAttachments(uuid, files.value);
    }
