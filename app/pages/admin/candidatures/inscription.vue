<!-- pages/admin/candidatures/inscription.vue -->
      ...
      <div v-if="etapeActive === 0" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div v-if="champsConfig.nom_jeune_fille?.afficher">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {{ champsConfig.nom_jeune_fille.label }}
                <span v-if="champsConfig.nom_jeune_fille.obligatoire" class="text-red-500">*</span>
                <span v-else class="text-gray-400 text-xs">(optionnel)</span>
              </label>
              <input v-model="formData.nom_jeune_fille" type="text" :required="champsConfig.nom_jeune_fille.obligatoire" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500" placeholder="Nom de jeune fille (si applicable)" />
            </div>
            ...
            <div v-if="champsConfig.numero_bordereau?.afficher">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {{ champsConfig.numero_bordereau.label }}
                <span v-if="champsConfig.numero_bordereau.obligatoire" class="text-red-500">*</span>
                <span v-else class="text-gray-400 text-xs">(optionnel)</span>
              </label>
              <input v-model="formData.numero_bordereau" type="text" :required="champsConfig.numero_bordereau.obligatoire" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500" :placeholder="champsConfig.numero_bordereau.label" />
            </div>
            ...
            <div class="md:col-span-2" v-if="champsConfig.comment_connu_ecole?.afficher">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Comment avez-vous connu {{ sigleEtablissement || "l'établissement" }} ?
                <span v-if="champsConfig.comment_connu_ecole.obligatoire" class="text-red-500">*</span>
                <span v-else class="text-gray-400 text-xs">(optionnel)</span>
              </label>
              <select v-model="formData.moyen_connaissance_id" :required="champsConfig.comment_connu_ecole.obligatoire" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500">
                <option value="">Sélectionnez</option>
      ...
      <div v-if="etapeActive === 1" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div v-if="champsConfig.tel2?.afficher">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {{ champsConfig.tel2.label }}
                <span v-if="champsConfig.tel2.obligatoire" class="text-red-500">*</span>
                <span v-else class="text-gray-400 text-xs">(optionnel)</span>
              </label>
            ...
            <div v-if="champsConfig.tel3?.afficher">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {{ champsConfig.tel3.label }}
                <span v-if="champsConfig.tel3.obligatoire" class="text-red-500">*</span>
                <span v-else class="text-gray-400 text-xs">(optionnel)</span>
              </label>
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select v-model="formData.type_diplome_id" required class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500">
                <option value="">Sélectionnez</option>
                <option v-for="type in typesDiplomeActifs" :key="type.id" :value="type.id">{{ type.nom }}</option>
              </select>
            </div>

            <div v-if="'numero_table' in champsParcoursConfig">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Numéro de table <span v-if="champsParcoursConfig.numero_table" class="text-red-500">*</span>
              </label>
              <input
                ...
                pattern="[0-9]*"
                maxlength="7"
                :required="champsParcoursConfig.numero_table"
            ...
            <div v-if="'annee_bac' in champsParcoursConfig">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Année du BAC <span v-if="champsParcoursConfig.annee_bac" class="text-red-500">*</span>
              </label>
              <input v-model="formData.annee_bac" type="number" min="1990" :max="anneeCourante" :required="champsParcoursConfig.annee_bac" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500" placeholder="Année" />
            </div>

            <div v-if="'serie' in champsParcoursConfig">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Série du BAC <span v-if="champsParcoursConfig.serie" class="text-red-500">*</span>
              </label>
              <select v-model="formData.serie" :required="champsParcoursConfig.serie" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500">
                <option value="">Sélectionnez</option>
            ...
            <div v-if="'mention_bac' in champsParcoursConfig">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Mention au BAC <span v-if="champsParcoursConfig.mention_bac" class="text-red-500">*</span>
              </label>
              <select v-model="formData.mention_bac" :required="champsParcoursConfig.mention_bac" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500">
                <option value="">Sélectionnez</option>
            ...
            <div v-if="'etablissement_diplome' in champsParcoursConfig">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Établissement <span v-if="champsParcoursConfig.etablissement_diplome" class="text-red-500">*</span>
              </label>
              <input v-model="formData.etablissement_diplome" type="text" :required="champsParcoursConfig.etablissement_diplome" class="w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500" placeholder="Établissement" />
            </div>
          </div>
        </div>
...
const moyenConnaissanceStore = useMoyenConnaissanceStore()
const candidatureFieldConfigStore = useCandidatureFieldConfigStore()
...
// Widgets d'indicatif téléphonique (mêmes options que celles déjà utilisées ailleurs
// dans l'admin, ex. liste-des-etudiants/ajouter.vue). Comme les étapes du formulaire se
// démontent/remontent (v-if), on synchronise le numéro complet dans le formulaire à
// chaque changement (blur/countrychange) plutôt qu'une seule fois à l'envoi, sinon le
// widget des étapes déjà quittées n'existerait plus.
const phoneWidgets = {}
...
// Reflète le mapping $mapKeyForUpload de CandidatureController::updateOrCreateAlbum, pour
// que les champs dynamiques restent compatibles avec le même backend que le formulaire public.
...
// Données du formulaire — mêmes champs que le formulaire public de dépôt.
...
// Champs configurables (afficher/obligatoire/label) définis dans Paramètres >
// Champs obligatoires — mêmes données que le formulaire public de dépôt.
const champsConfig = computed(() => {
  const map = {}
  ;(candidatureFieldConfigStore.champs || []).forEach(c => {
    map[c.champ_key] = { obligatoire: !!c.obligatoire, afficher: !!c.afficher, label: c.label }
  })
  return map
})
...
// Champs du parcours scolaire à afficher/exiger pour le type de diplôme sélectionné :
// { [champ_key]: obligatoire (bool) }. Un champ absent de cette map n'est pas affiché.
const champsParcoursConfig = computed(() => {
  const type = typesDiplomeActifs.value.find(t => t.id === Number(formData.type_diplome_id))
  if (!type) return {}
  const map = {}
...
// Tuteurs répétables (au moins un par défaut). `_uid` est un identifiant stable et
// unique par tuteur (indépendant de sa position dans le tableau) : nécessaire pour que
// `:key` ne réutilise jamais le même nœud DOM après un ajout/retrait, ce qui casserait
// sinon l'association entre le widget d'indicatif téléphonique et le bon tuteur.
...
// documents requis pour ce niveau (même principe que le formulaire public, généralisé
// à n'importe quel niveau au lieu du seul "Licence 1").
...
// Initialise/nettoie documentFiles à chaque changement de la liste de documents
// applicables, pour que le v-model de chaque champ dynamique soit toujours défini.
...
onMounted(async () => {
  try {
    ...
    if (typesDiplomeActifs.value.length && !formData.type_diplome_id) {
      formData.type_diplome_id = typesDiplomeActifs.value[0].id
    }

    await Promise.all([
      moyenConnaissanceStore.fetchMoyensConnaissance(),
      candidatureFieldConfigStore.fetchChamps(),
      parametreStore.fetchParametres(),
    ])
  } catch (error) {
    console.error('Erreur chargement données:', error)
    toast.add({ severity: 'error', summary: 'Erreur', detail: 'Impossible de charger les niveaux', life: 5000 })
  }
})

// Vérifie que tous les champs obligatoires (identité, contact, parcours, tuteurs,
// documents) sont bien remplis avant l'envoi — même logique que le formulaire public.
const validerAvantEnvoi = () => {
  ...
  if (!formData.type_diplome_id) manquants.push('Type du dernier diplôme')
  if (champsParcoursConfig.value.numero_table && (!formData.numero_table || formData.numero_table.length > 7)) manquants.push('Numéro de table (1 à 7 chiffres)')
  if (champsParcoursConfig.value.annee_bac && !formData.annee_bac) manquants.push('Année du BAC')
  if (champsParcoursConfig.value.serie && !formData.serie) manquants.push('Série du BAC')
  if (champsParcoursConfig.value.mention_bac && !formData.mention_bac) manquants.push('Mention au BAC')
  ...
  applicableDocs.value.forEach(doc => {
    ...
    const manque = doc.is_multiple ? !value || value.length === 0 : !value
    if (manque) manquants.push(doc.nom_affichage)
  })
...
const soumettreFormulaire = async () => {
  ...
  try {
    ...
    if (response.success) {
      toast.add({ severity: 'success', summary: 'Succès', detail: 'Candidature enregistrée avec succès', life: 5000 })
      setTimeout(() => {
