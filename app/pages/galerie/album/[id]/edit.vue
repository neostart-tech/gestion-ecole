  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-6 transition-colors">
    ...
    <div v-else class="max-w-3xl mx-auto">
      ...
      <nav class="mb-6 flex items-center gap-2 text-sm text-gray-400 dark:text-gray-500">
        ...
        <span class="text-gray-600 dark:text-gray-300">Modifier l'album</span>
      </nav>
      ...
      <div class="mb-6">
        <div class="flex items-center gap-3 mb-1">
          ...
          <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Modifier l'album</h1>
        </div>
        <p class="text-sm text-gray-400 dark:text-gray-500 pl-12">
          Mise à jour de <strong>{{ form.name }}</strong>
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div>
          <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-6">
            ...
            >
              <!-- Prévisualisation -->
              <div v-if="previewUrl || form.cover_url" class="w-full">
                <img :src="previewUrl || form.cover_url" alt="Couverture" class="w-full h-36 object-cover rounded-lg mb-3"/>
            ...
            <div class="mt-4 bg-violet-50 dark:bg-violet-950/30 border border-violet-100 dark:border-violet-900/40 rounded-lg p-3 flex gap-2">
              ...
              <p class="text-xs text-violet-700 dark:text-violet-300 leading-relaxed">
                Laissez vide pour conserver l'image actuelle. JPG, PNG, WEBP · max 5 Mo.
              </p>
            </div>
          </div>
        </div>
...
const form = ref({ name: "", description: "", cover: null, cover_url: "" });
...
onMounted(async () => {
  try {
    const album = await galerieStore.fetchAlbumById(albumId);
    if (album) {
      form.value.name = album.name;
      form.value.description = album.description || "";
      form.value.cover_url = album.cover_url;
    }
  } catch (error) {
    console.error("Erreur chargement album:", error);
    toast.add({ severity: "error", summary: "Erreur", detail: "Impossible de charger l'album", life: 3000 });
    router.push("/galerie");
  } finally {
...
const sauvegarder = async () => {
  ...
  try {
    ...
    await galerieStore.updateAlbum(albumId, formData);
