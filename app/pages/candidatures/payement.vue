<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-6 transition-colors">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
      ...
      <span>/</span>
      <span class="text-gray-900 dark:text-white font-medium cursor-default">Payement des frais de participation</span>
    </div>

    <!-- Titre -->
    <h1 class="text-2xl md:text-3xl font-semibold text-gray-900 dark:text-white mb-6">Payement des frais de participation</h1>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
      ...
      <Can action="payer-participation-candidature">
        <button
          v-if="selectedSlugs.length"
          @click="markSelectedAsPaid"
          :disabled="isSubmitting"
          class="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium text-sm disabled:opacity-50 shadow-sm"
        >
          ...
          <thead class="bg-gray-50 dark:bg-gray-900/50">
            <tr>
              <th class="px-6 py-4 w-10">
                <input type="checkbox" :checked="allSelected" @change="toggleSelectAll" class="rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-indigo-600 focus:ring-indigo-500" />
              </th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="filteredCandidates.length === 0">
              <td colspan="5" class="px-6 py-12 text-center">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Aucun candidat en attente de paiement</h3>
            ...
            <tr v-for="candidate in filteredCandidates" :key="candidate.slug" class="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
              <td class="px-6 py-4">
                <input type="checkbox" v-model="selectedSlugs" :value="candidate.slug" class="rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-indigo-600 focus:ring-indigo-500" />
              </td>
              <td class="px-6 py-4">
                ...
                <div class="flex justify-center">
                  <Can action="payer-participation-candidature">
                    <button
                      ...
                      :disabled="isSubmitting"
                      title="Marquer comme ayant payé"
                      class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white transition-colors disabled:opacity-50"
                    >
...
const filteredCandidates = computed(() => {
  ...
  return candidates.value.filter(candidate =>
    ...
    String(candidate.numero_dossier_affiche).toLowerCase().includes(query) ||
