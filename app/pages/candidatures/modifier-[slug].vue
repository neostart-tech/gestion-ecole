...
const chargerCandidature = async () => {
  ...
  try {
    ...
    // Candidature créée avant l'introduction de type_diplome_id : on retrouve le type
    // par son nom (dernier_diplome, dénormalisé) pour préremplir la sélection.
    if (!formData.type_diplome_id && data.dernier_diplome) {
      const typeCorrespondant = typeDiplomeStore.typesDiplome.find(t => t.nom === data.dernier_diplome)
      if (typeCorrespondant) formData.type_diplome_id = typeCorrespondant.id
    }
