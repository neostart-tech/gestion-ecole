<template>
  <div class="min-h-screen bg-[#f3f3f8] dark:bg-[#08080f] font-sans transition-colors duration-500 relative">
    ...
    <div class="relative z-10 w-full p-3 sm:p-5 lg:p-8 space-y-5 sm:space-y-8">
      
      <!-- Header Intégré & Professionnel -->
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div class="flex items-center gap-3 mb-1 sm:mb-2">
            <div class="w-1.5 h-6 sm:h-8 bg-[#7F45FD] rounded-full"></div>
            <h1 class="text-xl sm:text-2xl lg:text-3xl font-black text-[#1a1a2a] dark:text-[#fafafe] tracking-tight">Étude des Dossiers</h1>
          </div>
          <p class="text-xs sm:text-sm text-[#8a8a9a] font-medium ml-4">Gestion centralisée et validation des candidatures entrantes</p>
        </div>

        <button
          @click="exportExcel"
          :disabled="candidatureStore.exportEnCours"
          class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 bg-[#7F45FD] hover:bg-[#6a35e8] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 hover:shadow-[0_8px_25px_rgba(127,69,253,0.3)] disabled:opacity-60 shrink-0"
        >
          ...
          {{ candidatureStore.exportEnCours ? 'Export...' : 'Exporter en Excel' }}
        </button>
      </div>

      <!-- Onglets Filtres (Segmented Control Horizontal Scroll Responsive) -->
      <div class="bg-[#ededf4] dark:bg-[#151525] p-1.5 rounded-2xl flex items-center gap-1.5 overflow-x-auto no-scrollbar border border-[#e8e8f0]/40 dark:border-[#1a1a2a]/40 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
        <button 
          @click="filterStatut = ''"
          :class="[
            filterStatut === '' 
          ...
          class="shrink-0 px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 transform active:scale-95 whitespace-nowrap"
        >
      ...
      <div class="bg-white dark:bg-[#11111e] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-2xl p-3 sm:p-4 lg:p-5 shadow-[0_8px_30px_rgba(127,69,253,0.04)] relative z-20">
        ...
        <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 sm:gap-4">
          ...
          <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            <div class="relative flex-1 min-w-0">
              <input
                ...
                class="w-full pl-10 sm:pl-11 pr-4 py-2.5 sm:py-3 text-xs sm:text-sm bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-xl focus:ring-2 focus:ring-[#7F45FD]/20 focus:border-[#7F45FD] outline-none transition-all text-[#1a1a2a] dark:text-white placeholder-[#8a8a9a]"
              />
              <svg class="w-4 h-4 sm:w-5 sm:h-5 absolute left-3.5 sm:left-4 top-3 sm:top-3.5 text-[#8a8a9a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>

            <!-- Sélecteur de colonnes -->
            <Menu as="div" class="relative shrink-0 hidden sm:block z-30">
              <MenuButton class="flex items-center justify-center gap-2 h-full px-3.5 sm:px-4 py-2.5 sm:py-3 bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-xl hover:bg-[#e8e8f0] dark:hover:bg-[#1a1a2a] transition-all text-[#1a1a2a] dark:text-[#fafafe] focus:outline-none">
                ...
                <span class="text-xs sm:text-sm font-medium">Colonnes</span>
              </MenuButton>
              <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                <MenuItems class="absolute right-0 mt-2 w-56 p-3 rounded-xl bg-white dark:bg-[#11111e] border border-[#e8e8f0] dark:border-[#1a1a2a] z-50 shadow-[0_8px_30px_rgba(0,0,0,0.15)] focus:outline-none">
          ...
          <div class="grid grid-cols-2 sm:flex sm:items-center gap-2 sm:gap-3 shrink-0">
            <select v-model="filterFiliere" class="w-full sm:w-44 px-3.5 py-2.5 sm:py-3 text-xs sm:text-sm bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-xl outline-none focus:border-[#7F45FD] text-[#1a1a2a] dark:text-white truncate">
              <option value="">Filières</option>
              <option v-for="f in filiereStore.filieres" :key="f.id" :value="f.id">{{ f.nom }}</option>
            </select>

            <select v-model="filterNiveau" class="w-full sm:w-44 px-3.5 py-2.5 sm:py-3 text-xs sm:text-sm bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-xl outline-none focus:border-[#7F45FD] text-[#1a1a2a] dark:text-white truncate">
              <option value="">Niveaux</option>
              <option v-for="n in niveauStore.niveaux" :key="n.id" :value="n.id">{{ n.nom || n.libelle }}</option>
            </select>

            <button @click="refreshData" title="Actualiser" class="col-span-2 sm:col-span-1 shrink-0 p-2.5 sm:p-3 bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] text-[#8a8a9a] hover:text-[#7F45FD] rounded-xl shadow-sm transition-all flex items-center justify-center">
      ...
      <div v-if="isLoading" class="space-y-4">
        <div v-for="i in 5" :key="i" class="bg-white dark:bg-[#11111e] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-2xl p-4 sm:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-pulse shadow-sm">
          <div class="flex items-center gap-4 flex-1">
            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-gray-200 dark:bg-gray-800/50 rounded-xl"></div>
            <div class="space-y-2 sm:space-y-3">
              <div class="h-4 bg-gray-200 dark:bg-gray-800/50 rounded w-36 sm:w-48"></div>
              <div class="h-3 bg-gray-200 dark:bg-gray-800/50 rounded w-24 sm:w-32"></div>
            </div>
          </div>
      ...
      <!-- LISTE DES DOSSIERS (Responsive : Vue Cartes sur Mobile/Tablette < lg, Vue Table sur Desktop >= lg) -->
      <template v-else-if="filteredCandidatures.length > 0">
        
        <!-- VUE CARTES (Mobile et Tablette < lg) -->
        <div class="block lg:hidden space-y-3 sm:space-y-4">
          <div 
            v-for="c in paginatedCandidatures" 
            :key="c.id"
            class="bg-white dark:bg-[#11111e] rounded-2xl p-4 sm:p-5 border border-[#e8e8f0] dark:border-[#1a1a2a] shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:border-[#7F45FD]/30 transition-all space-y-4"
          >
            <!-- En-tête de la carte : Photo + Nom + Statut -->
            <div class="flex items-start justify-between gap-3 pb-3 border-b border-[#e8e8f0]/60 dark:border-[#1a1a2a]/60">
              <div class="flex items-center gap-3 min-w-0">
                <div class="w-10 h-10 rounded-xl bg-[#7F45FD]/10 flex items-center justify-center text-[#7F45FD] font-black text-xs shrink-0 relative overflow-hidden">
                  ...
                  <img v-if="c.album?.photo" :src="getFullUrl(c.album.photo)" class="absolute inset-0 w-full h-full object-cover" />
                </div>
                <div class="flex flex-col min-w-0">
                  <span class="text-xs sm:text-sm font-bold text-[#1a1a2a] dark:text-[#fafafe] truncate uppercase leading-tight">
                    {{ c.nom }} <span class="text-[#7F45FD]">{{ c.prenom }}</span>
                  </span>
                  <span class="text-[11px] text-[#8a8a9a] truncate font-medium mt-0.5">{{ c.email }}</span>
                </div>
              </div>

              <!-- Statut -->
              <span :class="getStatutBadgeClass(c)" class="px-2.5 py-1 rounded-xl border text-[9px] font-bold uppercase tracking-[0.05em] shrink-0">
                {{ getStatutLabel(c) }}
              </span>
            </div>

            <!-- Détails : Grille 2 Colonnes -->
            <div class="grid grid-cols-2 gap-3 text-xs">
              <div class="flex flex-col">
                <span class="text-[10px] font-bold uppercase tracking-wider text-[#8a8a9a]">Dossier N°</span>
                <span class="font-mono font-semibold text-[#1a1a2a] dark:text-[#fafafe] mt-0.5">{{ c.numero_dossier_affiche || '—' }}</span>
              </div>

              <div class="flex flex-col">
                <span class="text-[10px] font-bold uppercase tracking-wider text-[#8a8a9a]">Filière</span>
                <span class="font-bold text-[#1a1a2a] dark:text-[#fafafe] truncate mt-0.5">{{ c.filiere?.nom || '—' }}</span>
              </div>

              <div class="flex flex-col col-span-2 sm:col-span-1">
                <span class="text-[10px] font-bold uppercase tracking-wider text-[#8a8a9a]">Niveau</span>
                <span class="font-bold text-[#7F45FD] truncate mt-0.5">{{ c.niveau?.nom || c.niveau?.libelle || '—' }}</span>
              </div>
            </div>

            <!-- Actions de la carte -->
            <div class="flex items-center justify-end gap-2 pt-2 border-t border-[#e8e8f0]/60 dark:border-[#1a1a2a]/60">
              <NuxtLink 
                :to="`/candidatures/modifier-${c.slug || c.id}`"
                class="flex-1 sm:flex-none text-center px-4 py-2.5 bg-[#fafafe] dark:bg-[#0a0a12] text-[#1a1a2a] dark:text-[#fafafe] text-[10px] font-bold rounded-xl hover:bg-[#e8e8f0] dark:hover:bg-[#1a1a2a] transition-all uppercase tracking-widest border border-[#e8e8f0] dark:border-[#1a1a2a]"
              >
                Éditer
              </NuxtLink>
              <NuxtLink 
                :to="`/candidatures/${c.slug || c.id}`"
                class="flex-1 sm:flex-none text-center px-5 py-2.5 bg-[#7F45FD] text-white text-[10px] font-bold rounded-xl hover:bg-[#6a35e8] transition-all uppercase tracking-widest shadow-sm"
              >
                Ouvrir
              </NuxtLink>
            </div>
          ...
          <!-- Pagination Mobile -->
          <div v-if="totalPages > 1" class="flex items-center justify-between bg-white dark:bg-[#11111e] rounded-2xl p-4 border border-[#e8e8f0] dark:border-[#1a1a2a]">
            <button
              @click="currentPage > 1 && currentPage--"
              :disabled="currentPage === 1"
              class="px-3.5 py-2 text-xs font-bold bg-[#fafafe] dark:bg-[#0a0a12] border border-[#e8e8f0] dark:border-[#1a1a2a] rounded-xl text-[#1a1a2a] dark:text-white disabled:opacity-40"
            >
              Précédent
            </button>
            <span class="text-xs font-semibold text-[#8a8a9a]">Page {{ currentPage }} / {{ totalPages }}</span>
            <button
        ...
        <!-- VUE DATATABLE (Desktop >= lg) -->
        <div class="hidden lg:block bg-white dark:bg-[#11111e] rounded-2xl border border-[#e8e8f0] dark:border-[#1a1a2a] shadow-[0_8px_30px_rgba(127,69,253,0.04)] overflow-hidden p-4 lg:p-6">
          <vue3-datatable
      ...
      <!-- Vide -->
      <div v-else class="py-16 sm:py-20 text-center bg-white dark:bg-[#11111e] rounded-2xl border border-[#e8e8f0] dark:border-[#1a1a2a] px-4">
          <div class="w-14 h-14 sm:w-16 sm:h-16 bg-[#7F45FD]/5 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg class="w-7 h-7 sm:w-8 sm:h-8 text-[#7F45FD]/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4" />
            </svg>
          </div>
          <h3 class="text-base sm:text-lg font-bold text-[#1a1a2a] dark:text-[#fafafe] mb-2">Aucun dossier</h3>
          <p class="text-xs sm:text-sm font-medium text-[#8a8a9a]">Aucun résultat trouvé pour votre recherche.</p>
      </div>
...
// Pagination pour la vue mobile (cartes)
const currentPage = ref(1)
const pageSize = ref(10)

const totalPages = computed(() => Math.ceil(filteredCandidatures.value.length / pageSize.value) || 1)

const paginatedCandidatures = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value
  return filteredCandidatures.value.slice(start, start + pageSize.value)
})

watch([searchQuery, filterFiliere, filterNiveau, filterStatut], () => {
  currentPage.value = 1
})
...
.dark :deep(.bh-pagination) {
  border-top-color: #1a1a2a !important;
...
:deep(.bh-pagination .bh-page-item.active) {
  background-color: #7F45FD !important;
