      ...
      <div class="lg:col-span-2 bg-white dark:bg-[#11111e] rounded-[10px] p-8 shadow-xl border border-gray-100 dark:border-[#1a1a2a] relative overflow-hidden">
        ...
        <div v-else class="relative">
          ...
          <div class="space-y-8 relative">
            <div v-for="etape in etapesTimeline" :key="etape.key" class="flex gap-4 group" :class="etape.statut === 'attente' ? 'opacity-50' : ''">
              ...
              <div class="flex-1 pt-1">
                ...
                <div v-if="etape.statut === 'alerte'" class="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[10px] bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800">
        ...
        <div class="bg-white dark:bg-[#11111e] rounded-[10px] p-6 shadow-xl border border-gray-100 dark:border-[#1a1a2a] text-center flex flex-col justify-center min-h-[200px] relative overflow-hidden group">
          ...
          <div class="relative z-10">
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">
              {{ candidature?.admission ? "Admis(e) !" : (isRectification ? 'Vérification demandée' : (candidature?.dossier_valide ? 'En attente de décision' : "En cours d'étude")) }}
            </h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">
              {{ candidature?.admission ? `Félicitations, vous êtes admis(e) à ${appName}.` : (isRectification ? 'Des corrections sont attendues de votre part avant de poursuivre l\'étude de votre dossier.' : 'Votre dossier est actuellement entre les mains de notre équipe.') }}
            </p>
          </div>
        </div>
...
// Le concours (épreuve écrite) n'est actif que pour la session de concours à
// laquelle cette candidature est rattachée — sinon c'est le mode "dossier
// simple", où l'étude de conformité vaut directement décision d'admission
// (voir CandidatureFirstValidationTrait::candidatureSuitLeModeConcours côté backend).
const estModeConcours = computed(() => !!candidature.value?.concours_session?.avec_epreuve_ecrite);

// Étapes affichées dans la timeline : construites dynamiquement à partir des
// vrais champs de statut du dossier, pour rester fidèles au circuit réel
// (transmission à l'académie, puis mode dossier OU mode concours avec ses
// étapes propres avant de rejoindre l'admission/l'inscription).
const etapesTimeline = computed(() => {
  const c = candidature.value;
  if (!c) return [];
  ...
  const etapes = [
    {
      ...
      description: "Vos informations et documents ont été reçus avec succès sur notre plateforme.",
      statut: "termine",
      badge: `Soumis le ${formatDate(c.created_at)}`,
    },
    {
      // La demande de rectification n'est possible qu'avant la transmission
      // à l'académie (action du chargé de la clientèle) : c'est donc ici,
      // et pas à l'étape suivante, qu'il faut signaler une correction en attente.
      key: "transmission",
      titre: "Transmission à l'académie",
      description: "Votre dossier est transféré au service académique pour étude.",
      statut: isRectification.value ? "alerte" : c.transmis_academie ? "termine" : "en_cours",
    },
  ];

  etapes.push({
    key: "conformite",
    titre: "Étude de conformité",
    description: "Vérification de l'authenticité de vos pièces jointes par notre comité académique.",
    statut: !c.transmis_academie ? "attente" : c.dossier_valide ? "termine" : "en_cours",
    badge: "Dossier validé et conforme",
  });

  if (estModeConcours.value) {
    etapes.push({
      key: "frais",
      titre: "Paiement des frais de concours",
      description: "Règlement des frais de participation à l'épreuve écrite.",
      statut: !c.dossier_valide ? "attente" : c.frais_paye ? "termine" : "en_cours",
      badge: "Frais réglés",
    });
    etapes.push({
      key: "participation",
      titre: "Participation à l'épreuve",
      description: "Présence enregistrée le jour de l'épreuve écrite du concours.",
      statut: !c.frais_paye ? "attente" : c.participation ? "termine" : "en_cours",
      badge: "Présence enregistrée",
    });
  }

  const etapePrecedenteValidee = estModeConcours.value ? c.participation : c.dossier_valide;
  etapes.push({
    key: "admission",
    titre: "Décision d'Admission",
    description: "Approbation finale de votre candidature par la direction académique et jury.",
    statut: !etapePrecedenteValidee ? "attente" : c.admission ? "termine" : "en_cours",
    badge: "Admis(e)",
  });

  etapes.push({
    key: "inscription",
    titre: "Inscription officielle",
    description: "Validation de l'inscription et création de votre espace étudiant définitif.",
    statut: !c.admission ? "attente" : c.etudiant_id ? "termine" : "en_cours",
    badge: "Inscription finalisée",
  });
