      ...
      <div class="bg-white dark:bg-[#11111e] rounded-[10px] p-6 sm:p-8 border border-gray-100 dark:border-[#1a1a2a] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
          ...
          <div v-if="!isRectification || 'numero_table' in champsParcoursConfig">
            <label class="block text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">Numéro de table</label>
          ...
          <div v-if="!isRectification || 'mention_bac' in champsParcoursConfig">
            <label class="block text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">Mention</label>
        ...
        <div class="space-y-6">
          <div v-for="(tuteur, index) in tuteurs" :key="tuteur._uid" class="border border-gray-100 dark:border-[#1a1a2a] rounded-[10px] p-5 relative bg-gray-50/40 dark:bg-[#0a0a12]/30">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-5">
              ...
              <div>
                <label class="block text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5">Statut</label>
                <label
                  class="flex items-center gap-2.5 cursor-pointer select-none px-4 py-3 rounded-[10px] border h-[46px]"
                  :class="[
                    isRectification
                      ? 'bg-white dark:bg-[#1a1a2a] border-gray-200 dark:border-[#242438]'
                      : 'bg-gray-50/50 dark:bg-[#0a0a12]/50 border-gray-100 dark:border-[#1a1a2a]',
...
const buildUpdatePayload = () => ({
  ...
  nom_jeune_fille: formData.value.nom_jeune_fille,
  ...
  tel3: formData.value.tel3,
  email: formData.value.email,
  numero_bordereau: formData.value.numero_bordereau,
  moyen_connaissance_id: formData.value.moyen_connaissance_id,
  numero_table: formData.value.numero_table,
  serie: formData.value.serie,
  mention_bac: formData.value.mention_bac,
  annee_bac: formData.value.annee_bac,
  type_diplome_id: formData.value.type_diplome_id,
...
onMounted(async () => {
  await fetchCandidatureConfig();
  try {
    candidature.value = await candidatAuthStore.fetchMonDossier();
    if (candidature.value) {
      initFormData(candidature.value);

      // Candidature créée avant l'introduction de type_diplome_id (ou type non
      // reconnu) : on retrouve le type par son nom (dernier_diplome, dénormalisé) ;
      // à défaut, on retombe sur le premier type disponible plutôt que de laisser
      // le select vide.
      if (!formData.value.type_diplome_id && typesDiplome.value.length) {
        const typeCorrespondant = typesDiplome.value.find((t) => t.nom === formData.value.type_diplome);
        formData.value.type_diplome_id = typeCorrespondant ? typeCorrespondant.id : typesDiplome.value[0].id;
      }
