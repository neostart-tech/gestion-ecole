    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div v-for="doc in documents" :key="doc.id"
        ...
        <div class="flex items-start gap-5 mb-5">
          ...
          <div class="w-14 h-14 rounded-[10px] flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-105"
               :class="doc.fourni ? getFileIconColor(doc.extension) : 'bg-gray-200/50 dark:bg-gray-700/50 text-gray-400'">
          ...
          <div class="flex-1 min-w-0 pt-1">
            ...
            <div class="flex items-center gap-2">
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[10px] text-[10px] font-bold uppercase tracking-widest" :class="getStatusBadgeClass(doc.statut)">
        ...
        <div class="flex items-center justify-between mt-auto pt-4 border-t" :class="doc.fourni ? 'border-gray-50 dark:border-[#1a1a2a]' : 'border-gray-200 dark:border-[#242438] border-dashed'">
            <p class="text-xs font-medium" :class="doc.fourni ? 'text-gray-400' : 'text-rose-500/80'">
              <template v-if="doc.fourni">
                ...
                <span v-if="doc.date_ajout" class="px-1 text-gray-300 dark:text-gray-600">•</span>
           ...
           <div class="flex items-center gap-2">
             <!-- Bouton Télécharger : un seul fichier -->
             <button v-if="doc.fourni && doc.urls.length === 1" @click="downloadDoc(doc.urls[0])"
                     class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-[#01b4d5] bg-gray-50 dark:bg-[#1a1a2a] hover:bg-[#01b4d5]/10 rounded-[10px] transition-colors focus:ring-2 focus:ring-[#01b4d5]/30 focus:outline-none"
                     title="Télécharger le fichier">
               <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
             </button>

             <div v-if="isRectification">
               <input type="file" :ref="'fileInput_' + doc.id" class="hidden" :multiple="doc.is_multiple" @change="handleFileSelect($event, doc.id)" />
               <button @click="$refs['fileInput_' + doc.id][0].click()"
                       class="h-8 px-3 rounded-[10px] transition-all flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider focus:ring-2 focus:outline-none"
                       :class="selectedFiles[doc.id] ? 'text-indigo-700 dark:text-indigo-300 bg-indigo-100 dark:bg-indigo-900/40 hover:bg-indigo-200 dark:hover:bg-indigo-900/60 focus:ring-indigo-500/30 shadow-sm' : (doc.fourni ? 'text-[#01b4d5] bg-[#01b4d5]/10 hover:bg-[#01b4d5]/20 focus:ring-[#01b4d5]/30' : 'text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/40 hover:bg-emerald-200 dark:hover:bg-emerald-900/60 focus:ring-emerald-500/30 shadow-sm')">
               ...
               <p v-if="selectedFiles[doc.id]" class="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold mt-1 truncate max-w-[140px]" :title="selectedFilesLabel(doc)">{{ selectedFilesLabel(doc) }}</p>
             </div>
           </div>
        </div>

        <!-- Fichiers multiples déjà soumis : un lien de téléchargement par fichier -->
        <div v-if="doc.fourni && doc.is_multiple && doc.urls.length > 1" class="mt-3 pt-3 border-t border-dashed" :class="doc.fourni ? 'border-gray-50 dark:border-[#1a1a2a]' : 'border-gray-200 dark:border-[#242438]'">
          <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">{{ doc.urls.length }} fichiers soumis</p>
          <div class="flex flex-wrap gap-1.5">
            <a v-for="(url, idx) in doc.urls" :key="idx" :href="url" target="_blank"
               class="text-[10px] font-bold text-[#01b4d5] hover:underline px-2 py-1 bg-[#01b4d5]/10 rounded-[10px] transition-colors hover:bg-[#01b4d5]/20">
              Fichier {{ idx + 1 }}
            </a>
          </div>
        </div>

        <!-- Rappel : un document multi-fichiers se remplace en bloc -->
        <p v-if="isRectification && doc.is_multiple" class="mt-2 text-[10px] text-gray-400 dark:text-gray-500 italic">
          Document à plusieurs fichiers : sélectionnez-les tous ensemble, ils remplaceront l'ensemble précédent.
        </p>
      </div>
...
const isRectification = computed(() => {
  ...
  return !!candidatureState.value.rectification_expected;
});

// Formatage de la date et taille
...
const getFileIconColor = (ext) => {
  if (!ext) return "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400";
  const map = {
    pdf: "bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400",
    jpg: "bg-blue-50 dark:bg-blue-900/30 text-blue-500 dark:text-blue-400",
    png: "bg-blue-50 dark:bg-blue-900/30 text-blue-500 dark:text-blue-400",
    doc: "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400",
    docx: "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400",
  };
  return map[ext?.toLowerCase()] || "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400";
};

const getStatusBadgeClass = (statut) => {
  if (!statut) return "bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400";
  const s = statut.toLowerCase();
  if (s.includes("soumis") || s.includes("fourni")) return "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400";
  if (s.includes("manquant") || s.includes("non fourni")) return "bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400";
  if (s.includes("rectification")) return "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400";
...
// tableau encodé en JSON dans file_path (ex: '["documents/a.png","documents/b.png"]').
// On le décode en une vraie liste de chemins, sinon on retombe sur un tableau
// à un seul élément pour les documents simples.
...
const selectedFilesLabel = (doc) => {
  const sel = selectedFiles.value[doc.id];
  if (!sel) return "";
  if (Array.isArray(sel)) return `${sel.length} fichier(s) sélectionné(s)`;
  return sel.name;
};
...
const fetchDocuments = async () => {
  ...
  try {
    const data = await candidatAuthStore.fetchMesFichiers();
    const rawAlbum = data.documents || data.album || {};
    candidatureState.value = data.candidature || {};
    const expectedDocsList = data.expected_docs || [];
    const docMetadata = data.documents_metadata || {};
    ...
    if (expectedDocsList.length > 0) {
      for (const docReq of expectedDocsList) {
        ...
        const fourni = paths.length > 0;
        const ext = fourni ? (paths[0].split(".").pop() || "file") : "";
        ...
        docArray.push({
          ...
          is_multiple: !!docReq.is_multiple,
          ...
          urls: paths.map((p) => buildFileUrl(p)),
        });
      }
    }
...
const handleFileSelect = (event, documentKey) => {
  ...
  const doc = documents.value.find((d) => d.id === documentKey);
  selectedFiles.value[documentKey] = doc?.is_multiple ? Array.from(files) : files[0];
};
...
const submitRectifications = async () => {
  ...
  try {
    if (hasFiles) {
      ...
      for (const [key, fileOrFiles] of Object.entries(selectedFiles.value)) {
        const formData = new FormData();
        if (Array.isArray(fileOrFiles)) {
          fileOrFiles.forEach((f) => formData.append("document[]", f));
        } else {
          formData.append("document", fileOrFiles);
        }
