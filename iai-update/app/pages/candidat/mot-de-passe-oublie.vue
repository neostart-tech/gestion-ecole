      ...
      <div class="mx-auto w-full max-w-sm lg:w-96">
        ...
        <div v-if="success" class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-6 text-center">
          ...
          <p class="text-sm text-emerald-700 dark:text-emerald-400 font-medium">
            Si cet email existe, un lien de réinitialisation vient d'être envoyé à <strong>{{ email }}</strong>. Vérifiez votre boîte mail (et vos spams).
          </p>
        ...
        <div v-else class="mt-8">
          <form class="space-y-6" @submit.prevent="handleSubmit">
            ...
            <div v-if="errorMessage" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3 text-sm text-red-700 dark:text-red-400 font-medium">
              {{ errorMessage }}
            </div>
...
const handleSubmit = async () => {
  ...
  errorMessage.value = "";
  try {
    await candidatAuthStore.forgetPassword(email.value);
    success.value = true;
  } catch (error) {
    errorMessage.value = error.response?.data?.message || "Une erreur est survenue. Vérifiez votre adresse e-mail.";
  } finally {
