      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-100 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.enAttente }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-100 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.enCours }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-100 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ stats.termines }}</p>
          </div>
        ...
        >
          ...
          <template #group.nom="data">
            <span class="text-sm font-medium text-gray-900 dark:text-white">
              {{ data.value?.group?.nom || "Non spécifié" }}
            </span>
          </template>
            ...
            >
              ...
              >
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4">
                      ...
                      <div class="ml-11">
                        <span
                          ...
                          :class="selectedEvent.type === 'Examen' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' : 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300'"
                        >
                          {{ selectedEvent.type }}
                        </span>
                      </div>
                  ...
                  <div class="bg-gradient-to-br from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 rounded-xl p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-violet-100 dark:border-violet-700/30">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Durée</p>
                        <p class="text-lg font-bold text-violet-600 dark:text-violet-400 mt-1">
                          {{ formatDuration(selectedEvent.duration_minutes) }}
                        </p>
                      </div>
                      <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-violet-100 dark:border-violet-700/30">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Anonymat</p>
                        <div class="mt-1">
                          <span
                            class="px-3 py-1 rounded-full text-sm font-medium"
                            :class="selectedEvent.has_anonymat === 1 ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300' : 'bg-gray-100 dark:bg-gray-700/30 text-gray-800 dark:text-gray-300'"
                          >
                            {{ selectedEvent.has_anonymat === 1 ? "Activé" : "Désactivé" }}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                  ...
                  <NuxtLink
                    v-if="selectedEvent?.status === 'En attente' && selectedEvent?.is_online === 1"
                  ...
                  <NuxtLink
                    v-else-if="selectedEvent?.status === 'En cours' && selectedEvent?.is_online === 1"
...
const filteredRows = computed(() => {
  let filtered = rows.value.filter(e => e.published === 1 && e.is_online === 1);
