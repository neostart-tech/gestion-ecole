<template>
  <div class="min-h-screen bg-gray-50 p-4 md:p-6">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
      ...
      <span class="cursor-pointer hover:text-indigo-600 transition-colors">Filières</span>
      <span>/</span>
      <span class="text-gray-900 font-medium cursor-default">Liste</span>
    </div>

    <!-- Titre -->
    <h1 class="text-2xl md:text-3xl font-semibold text-gray-900 mb-6">Liste des filières</h1>
        ...
        <div class="modal-content">
          <form @submit.prevent="saveFiliere">
            ...
            <div class="modal-body p-6 space-y-4">
              ...
              <div class="form-group text-start">
                <label for="inscrits" class="block text-sm font-medium text-gray-700 mb-2">
                  Nombre d'inscrits
                </label>
                <input 
                  type="number" 
        ...
        <div class="modal-content">
          ...
          <div class="modal-body p-6">
            <p class="text-gray-600 mb-6">
              Voulez-vous vraiment supprimer cette filière ? Cette action est irréversible et affectera tous les étudiants associés. Continuer ?
            </p>
          </div>
          <div class="modal-footer border-t border-gray-200 p-6 flex justify-end gap-3">
            <form @submit.prevent="deleteFiliere" class="flex items-center gap-3">
              <input type="hidden" id="deleteFiliereForm" :value="selectedFiliere?.id" />
              <button 
                type="button" 
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(filiere => 
...
const deleteFiliere = () => {
  if (selectedFiliere.value) {
    const index = rows.value.findIndex(f => f.id === selectedFiliere.value.id);
    if (index !== -1) {
