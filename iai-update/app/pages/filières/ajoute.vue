<template>
  <div class="min-h-screen bg-gray-50 p-4 md:p-6">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
      ...
      <NuxtLink to="/filieres" class="cursor-pointer hover:text-indigo-600 transition-colors">Filières</NuxtLink>
      <span>/</span>
      <span class="text-gray-900 font-medium cursor-default">Ajouter une filière</span>
    </div>

    <!-- Titre -->
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl md:text-3xl font-semibold text-gray-900">Ajouter une filière</h1>
      <NuxtLink 
        to="/filieres" 
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div>
            ...
            />
            <p class="mt-1 text-sm text-gray-500">Ex: Génie Logiciel & Systèmes d'Informations</p>
          </div>

          <!-- Code de la filière -->
          <div>
            <label for="code" class="block text-sm font-medium text-gray-700 mb-2">
              Code de la filière <span class="text-rose-500">*</span>
            </label>
            <input
              id="code"
              v-model="form.code"
              type="text"
              required
              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all uppercase"
              placeholder="Code de la filière"
              maxlength="10"
            />
            <p class="mt-1 text-sm text-gray-500">Ex: GLSI, ASR, TC_1 (maximum 10 caractères)</p>
          </div>
          ...
          <div>
            ...
            ></textarea>
            <p class="mt-1 text-sm text-gray-500">Description détaillée de la filière (objectifs, débouchés, etc.)</p>
          </div>
                ...
                <div v-if="form.imagePreview" class="mt-4">
                  <div class="relative inline-block">
                    ...
                    <button
                      ...
                      class="absolute -top-2 -right-2 w-6 h-6 bg-rose-500 text-white rounded-full flex items-center justify-center hover:bg-rose-600"
                    >
            ...
            <div class="flex gap-4">
              ...
              <label class="inline-flex items-center">
                <input
                  ...
                  class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                />
                <span class="ml-2 text-sm text-gray-700">Bientôt disponible</span>
              </label>
            </div>
          </div>
...
const submitForm = async () => {
  ...
  try {
    ...
    const newId = filieres.length > 0 
      ? Math.max(...filieres.map(f => f.id)) + 1 
      : 1
