...
const programmesArray = computed(() => {
  if (Array.isArray(groupStore.programmes)) {
    return groupStore.programmes;
  }

  if (groupStore.programmes && typeof groupStore.programmes === "object") {
    ...
    for (const key of possibleArrayKeys) {
      if (Array.isArray(groupStore.programmes[key])) {
        return groupStore.programmes[key];
      }
    }

    for (const key in groupStore.programmes) {
      if (Array.isArray(groupStore.programmes[key])) {
        return groupStore.programmes[key];
      }
    }
...
const SallesOptions = computed(() => {
  return salleStore.salles.map((s: any) => {
    return {
      label: s.nom,
      value: s.id || s.slug,
    };
...
const fixSalleStore = () => {
  ...
  if (!groupStore.$state.programmes) {
    groupStore.$state.programmes = [];
  } else if (!Array.isArray(groupStore.$state.programmes)) {
    // Si c'est un objet, le convertir en tableau
    if (typeof groupStore.$state.programmes === "object") {
      // Essayer d'extraire un tableau
      const keys = Object.keys(groupStore.$state.programmes);
      for (const key of keys) {
        if (Array.isArray(groupStore.$state.programmes[key])) {
          groupStore.$state.programmes = groupStore.$state.programmes[key];
          break;
        }
      }
    }
