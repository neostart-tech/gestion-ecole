      ...
      >
        ...
        <template #roles="data">
          <div class="flex flex-wrap gap-1">
            <span 
              v-for="role in getRolesArray(data.value)" 
              :key="role"
              :class="[
          ...
          <div class="modal-body p-6 space-y-4">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <span 
                  v-for="role in getRolesArray(viewedMembre.roles)" 
                  :key="role"
                  :class="[
                    'px-2 py-1 rounded text-xs font-medium',
                    getRoleClass(role)
...
// Fonction pour convertir la chaîne de rôles en tableau
const getRolesArray = (rolesInput) => {
  if (!rolesInput) return [];
  if (Array.isArray(rolesInput)) {
    return rolesInput.filter(role => role && role.trim());
  }
  if (typeof rolesInput === 'string') {
    return rolesInput.split(' ').filter(role => role.trim());
  }
  return [];
};
...
const filteredRows = computed(() => {
  ...
  if (filterRole.value) {
    filtered = filtered.filter(membre => {
      const roles = getRolesArray(membre.roles);
      return roles.some(role => 
        role.toLowerCase().includes(filterRole.value.toLowerCase())
      );
    });
  }
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(membre => 
      ...
      membre.prenoms.toLowerCase().includes(query) ||
...
const openEditModal = (membre) => {
  ...
  form.value = { 
    ...
    email: membre.email,
    roles: getRolesArray(membre.roles),
    typeSurveillant: membre.typeSurveillant,
    actif: membre.actif,
...
// Gestion des membres
const saveMembre = () => {
  // S'assurer que form.roles est un tableau
  const rolesArray = Array.isArray(form.value.roles) 
    ? form.value.roles 
    : [form.value.roles];
  
  // Filtrer et convertir en string
  const rolesString = rolesArray
