...
const programmesArray = computed(() => {
  if (Array.isArray(userStore.programmes)) {
    return userStore.programmes;
  }

  if (userStore.programmes && typeof userStore.programmes === "object") {
    ...
    for (const key of possibleArrayKeys) {
      if (Array.isArray(userStore.programmes[key])) {
        return userStore.programmes[key];
      }
    }

    for (const key in userStore.programmes) {
      if (Array.isArray(userStore.programmes[key])) {
        return userStore.programmes[key];
      }
    }
...
onMounted(async () => {
  try {
    ...
    fixSalleStore();
    await userStore.userDisplayCalendar(slug.value);
    // await userStore.getSalleBySlug(slug.value);
    fixSalleStore();
